...
async fn create_expo_notifications_client() -> Expo {
    Expo::new(ExpoClientOptions { access_token: None })
}

pub async fn create_server(addr: &str) -> eyre::Result<Server> {
    let expo_notifications_client = create_expo_notifications_client().await;
